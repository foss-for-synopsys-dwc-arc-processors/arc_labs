...
or click the icon |icon2| . In the |mwdt| main interface, select **Console** to view the logs during compilation. When the message
'Finished building target: demo.elf' is displayed, the compilation is successful,
...
If you are using the EMSK development board, the terminal automatically selects the correct port number, and you are using the emulator without a port, uncheck it, as show in the following figure.
