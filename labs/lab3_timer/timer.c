...
int main(void)
{
    ...
    if (timer0_flag)
    {
		...
		uint32_t mode=(~TIMER_CTRL_IE) | TIMER_CTRL_NH;/*Timing without triggering interruption.*/
		uint32_t val=MAX_COUNT;
		_arc_aux_write(AUX_TIMER0_CNT, 0);
		_arc_aux_write(AUX_TIMER0_LIMIT,mode);
        ...
        uint32_t time=(end_cnt-start_cnt)*1000/BOARD_CPU_CLOCK;
